...
Shader "FlowerBloom" 
{
	...
	CGINCLUDE
		...
		fixed4 fragNoBloom(v2f_simple i) : SV_Target
		{
			#if UNITY_UV_STARTS_AT_TOP
			
			fixed4 color = tex2D(_MainTex, i.uv2);
			fixed4 screen = tex2D(_Screen, i.uv2);
			return lerp(screen, color, color.a);
			
			#else

			fixed4 color = tex2D(_MainTex, i.uv);
			fixed4 screen = tex2D(_Screen, i.uv);
			return lerp(screen, color, color.a);
			#endif
		}
